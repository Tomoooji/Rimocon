...
class Rimocon{
  ...
  public:
    ...
    //  void toggleFliped(int resultTgl[]);
    //  void volumeMoved(int resultVlm[]);
    //  void joystickTilted(Polar resultJstkTilte[]);
    //  void joystickPushed(bool resultJstkPush[]);

bool attach(uint8_t pinBtn[], uint8_t pinTgl[], uint8_t pinVlm[], uint8_t pinJstk[]){
  bool is_succesed;
  if(pinBtn != NULL){
...
void joystickTilted(Polar resultJstkTilt[]){
...
void detectAll(bool resultBtn[], int resultTgl[], int resultVlm[], Polar resultJstkTilte[], bool resultJstkPush[]){
